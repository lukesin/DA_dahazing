...
class SDehazingnetModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser.add_argument('--lambda_Dehazing', type=float, default=10.0,
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            self.visual_names = ['syn_haze_img', 'img_r2s', 'img_s2r', 'clear_img', 'r2s_dehazing_img', 'real_haze_img',
                                 's_dehazing_img']
        else:
    ...
    def backward_G(self):
        ...
        for (dehazing_img, clear_img) in zip(self.out[1:], clear_imgs):
            self.loss_S_Dehazing += self.criterionDehazing(dehazing_img[:self.num, :, :, :],
                                                           clear_img) * lambda_Dehazing

        # TV LOSS
        self.loss_R2S_Dehazing_TV = self.TVLoss(self.r2s_dehazing_img) * self.opt.lambda_Dehazing_TV

        # DC LOSS
        self.loss_R2S_Dehazing_DC = DCLoss((self.r2s_dehazing_img + 1) / 2,
