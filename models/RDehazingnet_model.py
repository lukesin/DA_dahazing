...
class RDehazingnetModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            self.visual_names = ['syn_haze_img', 'img_s2r', 'clear_img', 's2r_dehazing_img', 'real_haze_img',
                                 'r_dehazing_img']
    ...
    def backward_G(self):
        ...
        for (dehazing_img, clear_img) in zip(self.out[1:], clear_imgs):
            self.loss_S2R_Dehazing += self.criterionDehazing(dehazing_img[:self.num, :, :, :],
        ...
        # DC LOSS
        self.loss_R_Dehazing_DC = DCLoss((self.r_dehazing_img + 1) / 2,
